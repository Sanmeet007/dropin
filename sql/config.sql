...
VALUES (
        1,
        1000,
        'web development,UI/UX,Full Stack developer',
...
VALUES (
        ...
        'c0d922b3a29268975b481a6fea588da5',
        NULL,
        'hello world',
        NULL,
        'male',
...
(IN FIRST_NAME VARCHAR(255), IN LAST_NAME VARCHAR(
255), IN _EMAIL VARCHAR(255), IN PASSWORD VARCHAR(
255), IN LOCATION VARCHAR(255), IN BIO LONGTEXT, IN 
PROFILE_IMAGE VARCHAR(255), IN GENDER VARCHAR(255)
, IN ACCOUNT_TYPE VARCHAR(255), IN DOB DATE, IN COMPANY_NAME 
VARCHAR(255), IN COMPANY_WEBSITE VARCHAR(255), IN 
COMPANY_SIZE INT, IN INDUSTRY VARCHAR(255)) BEGIN 
	declare k int;
	...
	if _e is null then start transaction;
	INSERT INTO
	    users(
	        user_id,
	        first_name,
	...
	set
	    MESSAGE_TEXT = 'Email already registered';
	end if;
	END 
; 
...
(IN _PID INT) BEGIN 
	...
	select
	    ...
	    _jid
	from proposals
	where proposal_id = _pid;
	select freelancer_id into _fid
	from freelancers
	where user_id = _uid;
	if _fid is not null then start transaction;
	insert into
	    contracts (
	        freelancer_id,
	        job_id,
	...
	values (_fid, _jid, amnt, current_date);
...
255), IN LOCATION VARCHAR(255), IN BIO LONGTEXT, IN 
PROFILE_IMAGE VARCHAR(255), IN GENDER VARCHAR(255)
, IN ACCOUNT_TYPE VARCHAR(255), IN DOB DATE, IN SKILLS 
VARCHAR(255), IN PROGRAMMING_LANGUAGES VARCHAR(255
), IN DB VARCHAR(255), IN LANGS VARCHAR(255), IN OTHER_SKILLS 
VARCHAR(255), IN EDU VARCHAR(255)) BEGIN 
	declare k int;
	declare _e varchar(255);
	set k = (
	        select user_id + 1
...
(IN _USER_ID INT, IN TITLE VARCHAR(255), IN DESCRIPTION 
LONGTEXT, IN BUDGET DOUBLE, IN _SKILLSET TEXT) BEGIN 
	declare k int ;
	set k = (
	        select client_id
	        from clients
...
(IN _UID INT, IN _JID INT, IN COVER_LETTER LONGTEXT
, IN TIMEFRAME INT, IN BID_AMOUNT DOUBLE) BEGIN 
	declare _fid int;
	select freelancer_id into _fid
	from freelancers
	where user_id = _uid;
...
(IN _CID INT) BEGIN 
	...
	select
	    job_id,
	    freelancer_id,
	    payment_amount into _jid,
	    _fid,
	    _amnt
	from contracts
	where contract_id = _cid;
	select status into _job_status from jobs where job_id = _jid;
	if _job_status = 'closed' then start transaction ;
	update contracts
	set end_date = current_date
	where contract_id = _cid;
	insert into
	    payments (job_id, amount, status)
